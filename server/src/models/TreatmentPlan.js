...
const therapySessionSchema = new Schema({
  ...
  therapyType: {
    type: String,
    enum: ['snehana', 'swedana', 'abhyanga', 'vamana', 'virechana', 'basti', 'nasya', 'raktamokshana', 'samsarjana', 'shirodhara', 'pizhichil', 'udvartana', 'other','general'],
    required: true
  },
  ...
  materials: [{
    ...
    quantity: String,
    unit: { type: String, enum: ['ml', 'g', 'kg', 'units', 'drops', 'pieces'], default: 'ml' },
    batchNumber: String
  }],
...
TreatmentPlanSchema.virtual('estimatedCompletionDate').get(function() {
  if (!this.schedulingPreferences || !this.schedulingPreferences.startDate) return null;
  ...
  if (this.phases && this.phases.length > 0) {
    ...
    for (let i = 0; i < uniqueSequences.length; i++) {
      if (uniqueSequences[i] !== i + 1) {
        return next(new Error('Phase sequence must be continuous (1, 2, 3...)'));
      }
    }
  }
  next();
...
TreatmentPlanSchema.pre('save', function(next) {
  if (this.phases && this.phases.length > 0) {
    this.totalSessionsPlanned = this.phases.reduce((total, phase) => {
      return total + phase.therapySessions.reduce((phaseTotal, session) => {
...
TreatmentPlanSchema.methods.updateProgress = function() {
  if (this.totalSessionsPlanned > 0) {
    this.progress = Math.round((this.completedSessions / this.totalSessionsPlanned) * 100);
  }
  return this.save();
};
