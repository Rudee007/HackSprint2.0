...
createBooking = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await consultation.populate([
      { path: 'providerId', select: 'name email phone role' },
      { path: 'patientId', select: 'name email phone' }
    ]);

    console.log(`🎉 Booking created successfully: ${consultation._id}`);

    // ===== NOTIFICATION PHASE (patient + doctor) =====
    try {
      const patient = consultation.patientId;
      const doctor = consultation.providerId; // doctor/therapist
      ...
      console.warn('⚠️ Failed to create/send consultation notifications', {
        consultationId: consultation._id,
        error: notifErr.message
      });
      // booking stays successful even if notifications fail
    }
