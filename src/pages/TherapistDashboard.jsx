...
const TherapistDashboard = () => {
  ...
  const stats = useMemo(() => {
    return [
      { label: "Total Patients", value: String(therapyStats.totalPatients), icon: Users, color: "emerald" },
      { label: "Active Today", value: String(therapyStats.activeToday), icon: Activity, color: "blue", realTime: true },
      { label: "Completed", value: String(therapyStats.completed), icon: CheckCircle, color: "purple" },
      { label: "Improved", value: String(therapyStats.improved), icon: TrendingUp, color: "amber", realTime: true }
    ];
  }, [therapyStats]);
                    ...
                    <button
                      ...
                      className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl text-left transition-all duration-200 ${
                        activeSection === item.id
                          ...
                          : getInactiveNavStyles(item.color)
                      }`}
                    >
              ...
              {navigationItems.map((item, index) => (
                <motion.button
                  ...
                  className={`w-full flex items-center space-x-4 px-4 py-4 rounded-2xl text-left transition-all duration-300 group relative overflow-hidden ${
                    activeSection === item.id
                      ? getActiveDesktopNavStyles(item.color)
                      : getInactiveDesktopNavStyles(item.color)
                  }`}
                >
                  <div className={`p-2 rounded-xl transition-all ${activeSection === item.id ? "bg-white/20 shadow-lg" : getNavIconStyles(item.color)}`}>
                    <item.icon className="w-5 h-5" />
                  </div>
                  <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-6">
                    {stats.map((stat, i) => (
                      <motion.div
                        ...
                        className={`relative overflow-hidden p-4 lg:p-6 rounded-2xl lg:rounded-3xl shadow-lg border border-white/50 hover:shadow-xl transition-all duration-300 hover:scale-105 group cursor-pointer ${getStatBgGradient(stat.color)} ${stat.realTime ? "ring-2 ring-emerald-200 ring-opacity-50" : ""}`}
                      >
                        {stat.realTime && (
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex-1 min-w-0">
                            <p className={`text-xs lg:text-sm font-semibold mb-2 flex items-center space-x-2 ${getStatTextColor(stat.color)}`}>
                              <span className="truncate">{stat.label}</span>
                              {stat.realTime && <Circle className="w-2 h-2 text-emerald-500 fill-current animate-pulse flex-shrink-0" />}
                            </p>
                            <p className={`text-xl lg:text-3xl font-bold mb-1 ${getStatValueColor(stat.color)}`}>{stat.value}</p>
                          </div>
                          <div className={`w-10 h-10 lg:w-14 lg:h-14 rounded-xl lg:rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform flex-shrink-0 ml-2 ${getStatIconBg(stat.color)}`}>
