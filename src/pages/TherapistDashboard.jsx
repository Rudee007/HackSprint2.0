// src/pages/TherapistDashboard.jsx - 🔥 COMPLETE PRODUCTION-READY VERSION - EVERY FUCKING LINE 🔥
...
const getActiveNavStyles = (color) => {
  const colorMap = {
    ...
    amber: 'bg-gradient-to-r from-amber-500 to-amber-600 text-white shadow-lg',
...
const getInactiveNavStyles = (color) => {
  const colorMap = {
    ...
    purple: 'text-slate-600 hover:bg-purple-50 hover:text-purple-700',
...
const getActiveDesktopNavStyles = (color) => {
  const colorMap = {
    emerald: 'bg-gradient-to-r from-emerald-500 to-emerald-600 text-white shadow-xl scale-105',
    ...
    amber: 'bg-gradient-to-r from-amber-500 to-amber-600 text-white shadow-xl scale-105',
...
const getInactiveDesktopNavStyles = (color) => {
  const colorMap = {
    emerald: 'text-slate-600 hover:bg-emerald-50 hover:text-emerald-700 hover:scale-102',
    ...
    purple: 'text-slate-600 hover:bg-purple-50 hover:text-purple-700 hover:scale-102',
...
const getStatBgGradient = (color) => {
  const colorMap = {
    emerald: 'bg-gradient-to-br from-emerald-50 via-emerald-100 to-green-50',
    rose: 'bg-gradient-to-br from-rose-50 via-rose-100 to-pink-50',
    blue: 'bg-gradient-to-br from-blue-50 via-blue-100 to-cyan-50',
    purple: 'bg-gradient-to-br from-purple-50 via-purple-100 to-violet-50',
...
// ═══════════════════════════════════════════════════════════
// 🔥 MAIN COMPONENT - EVERY FUCKING LINE
// ═══════════════════════════════════════════════════════════
...
const TherapistDashboard = () => {
  ...
  const [feedback, setFeedback] = useState({ feedbacks: [], analytics: {} });
  ...
  // ═══════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════
// 🔥 TRACK LIVE HANDLER - NEW
// ═══════════════════════════════════════════════════════════

const handleTrackLive = useCallback((sessionId) => {
  console.log('🔴 [TRACK LIVE] Opening live tracking for session:', sessionId);
  ...
  toast('🔴 Opening live tracking...', { icon: '🔴', duration: 1500 });
...
const handleRefreshSessions = useCallback(async () => {
  try {
    ...
    const dashboardResponse = await therapistApiService.getDashboardOverview();
    
    if (dashboardResponse.success) {
      setDashboardData(dashboardResponse.data);
      setTodaysSessions(dashboardResponse.data?.todaysSessions || []);
      
      // Update active session
      const active = dashboardResponse.data?.activeSessions?.[0];
  ...
  useEffect(() => {
    // Get active session from your dashboard data
  ...
  useEffect(() => {
    const initWebSocket = async () => {
      ...
      try {
        ...
        const success = await therapistApiService.initializeWebSocket();
        setIsConnected(success);
        
        if (success) toast.success('🔴 LIVE - Real-time connected!');
      } catch (error) {
  ...
  // ═══════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════
// 🔥 LOAD DASHBOARD DATA - WITH FEEDBACK INTEGRATION
// ═══════════════════════════════════════════════════════════
...
useEffect(() => {
  const loadDashboardData = async () => {
    ...
    try {
      ...
      // Load profile
      const profileResponse = await therapistApiService.getTherapistProfile();
      if (!profileResponse.success) throw new Error('Failed to load profile');
      ...
      const patientsResponse = await therapistApiService.getAssignedPatients();
      if (patientsResponse.success) {
        setAssignedPatients(patientsResponse.data?.patients || []);
        console.log('✅ Patients loaded:', patientsResponse.data?.patients?.length);
      }

      // Load therapist stats
      const statsResponse = await therapistApiService.getTherapistStats('30d');
      if (statsResponse.success) {
        setStats(statsResponse.data);
        console.log('✅ Stats loaded');
      }

      // ✅ LOAD FEEDBACK (NEW - INTEGRATED)
      console.log('📊 Loading feedback data...');
      const feedbackResponse = await therapistApiService.getTherapistFeedback({
        page: 1,
        limit: 10,
        timeRange: '3months'
      });
      ...
      if (feedbackResponse.success) {
        ...
        setFeedback({
          ...
          analytics: {
            totalFeedback: feedbackData.pagination?.totalFeedback || 0,
            averageRating: feedbackData.averageRatings?.avgOverall || 0,
            positivePercentage: feedbackData.averageRatings?.avgOverall ? 
              (feedbackData.averageRatings.avgOverall / 5) * 100 : 0,
            ratingDistribution: {
              5: 0,
              4: 0,
              3: 0,
        ...
        console.log('✅ Feedback loaded:', {
          total: feedbackData.pagination?.totalFeedback,
          averageRating: feedbackData.averageRatings?.avgOverall
        });
      } else {
        console.warn('⚠️ Feedback loading failed:', feedbackResponse.error);
        // Set empty feedback data on error
        setFeedback({
          feedbacks: [],
          analytics: {
            totalFeedback: 0,
...
const refreshFeedback = useCallback(async () => {
  ...
  const handleStartSession = async (sessionId) => {
    try {
      const result = await therapistApiService.startSession(sessionId, 'Session started');
      
      if (result.success) {
        toast.success('🔴 LIVE - Session started!');
        const sessionsResponse = await therapistApiService.getTodaySessions();
        if (sessionsResponse.success) {
          setTodaysSessions(sessionsResponse.data?.sessions || []);
        }
      } else {
  ...
  const handleCompleteSession = async (sessionId, sessionData) => {
    try {
      const result = await therapistApiService.completeSession(sessionId, sessionData);
      ...
      if (result.success) {
        ...
        if (sessionsResponse.success) setTodaysSessions(sessionsResponse.data?.sessions || []);
  ...
  const handleUpdateProfile = async (formData) => {
    try {
      ...
      if (!therapistId) return { success: false, error: 'Therapist ID not found' };

      const result = await therapistApiService.updateTherapistProfile(therapistId, formData);
      
      if (result.success) {
        const profileResponse = await therapistApiService.getTherapistProfile();
        if (profileResponse.success) setTherapistProfile(profileResponse.data);
  ...
  const handleLogout = useCallback(() => {
    ...
    setTimeout(() => navigate('/therapist-login'), 1000);
  }, [navigate]);
  ...
  const displayName = useMemo(
    () => therapistProfile?.userId?.name || therapistProfile?.name || "Therapist",
    [therapistProfile]
  );
  ...
  const navigationItems = useMemo(() => ([
    ...
    { id: "sessions", label: "Today's Sessions", icon: Calendar, description: "Session management", color: "blue", badge: todaysSessions.length },
    { id: "feedback", label: "Feedback", icon: MessageCircle, description: "Patient reviews", color: "purple", badge: feedback.analytics?.totalFeedback || 0 },
    { id: "profile", label: "Profile", icon: User, description: "Therapist profile", color: "teal" }
  ]), [assignedPatients.length, todaysSessions.length, feedback.analytics?.totalFeedback]);
  ...
  const subTitleMap = {
    ...
    sessions: "View and manage your therapy sessions effectively",
    feedback: "Patient reviews and ratings",
    profile: "Customize your therapist profile and professional information"
  };

  const statsCards = useMemo(() => [
    { label: "Total Sessions", value: String(stats.totalSessions), icon: Users, color: "emerald" },
    { label: "Active Today", value: String(todaysSessions.filter(s => s.status === 'in_progress').length), icon: Activity, color: "blue", realTime: true },
    { label: "Completed", value: String(stats.completedSessions), icon: CheckCircle, color: "purple" },
    { label: "Average Rating", value: stats.averageRating?.toFixed(1) || '0.0', icon: Star, color: "amber", realTime: true }
      ...
      <div className="lg:hidden bg-white/95 backdrop-blur-sm border-b border-emerald-100 sticky top-0 z-50">
        <div className="flex items-center justify-between p-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button onClick={() => setMobileNavOpen(true)} className="p-2 rounded-xl bg-slate-100 hover:bg-slate-200">
                  ...
                  {navigationItems.map(item => (
                    <button
                      ...
                      className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl text-left transition-all ${
                        activeSection === item.id ? getActiveNavStyles(item.color) : getInactiveNavStyles(item.color)
                      }`}
                    >
            ...
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="mb-8 p-4 bg-gradient-to-r from-slate-50 to-emerald-50 rounded-2xl border border-slate-200/50 shadow-sm">
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center space-x-2">
                  <Circle className={`w-3 h-3 fill-current ${isConnected ? "text-emerald-500 animate-pulse" : "text-slate-400"}`} />
                  <span className="text-xs capitalize font-medium text-slate-700">{isConnected ? "available" : "offline"}</span>
                </div>
              </div>
            </motion.div>
              ...
              {navigationItems.map((item, index) => (
                <motion.button
                  ...
                  className={`w-full flex items-center space-x-4 px-4 py-4 rounded-2xl text-left transition-all duration-300 group ${
                    activeSection === item.id ? getActiveDesktopNavStyles(item.color) : getInactiveDesktopNavStyles(item.color)
                  }`}
                >
                  <div className={`p-2 rounded-xl transition-all ${activeSection === item.id ? "bg-white/20 shadow-lg" : `bg-${item.color}-100`}`}>
                    <item.icon className="w-5 h-5" />
                  </div>
                  <div className="flex-1">
                  ...
                  <ChevronRight className={`w-4 h-4 transition-all ${activeSection === item.id ? "rotate-90 text-white" : "text-slate-400"}`} />
                </motion.button>
              ))}
            ...
            <div className="flex flex-col xl:flex-row xl:items-center justify-between space-y-4 xl:space-y-0">
              <div className="flex-1">
                <AnimatePresence mode="wait">
                  <motion.h1 key={`title-${activeSection}`} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }} className="text-3xl font-bold bg-gradient-to-r from-slate-800 to-emerald-700 bg-clip-text text-transparent">
                    {titleMap[activeSection]}
                  </motion.h1>
                </AnimatePresence>
                <AnimatePresence mode="wait">
                  <motion.p key={`subtitle-${activeSection}`} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }} transition={{ delay: 0.1 }} className="text-slate-600 mt-2 text-lg">
                    {subTitleMap[activeSection]}
                  </motion.p>
                </AnimatePresence>
              </div>
              <AnimatePresence mode="wait">
                <motion.div key={`profile-${activeSection}`} initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="flex items-center space-x-4">
                  <div className="text-right">
                    <p className="text-lg font-semibold text-slate-800">{displayName}</p>
                    <p className="text-sm text-emerald-600 font-medium">{therapistProfile?.specialization?.[0] || "Ayurvedic Therapist"}</p>
            ...
            <AnimatePresence mode="wait">
              ...
              {activeSection === "overview" && (
                <motion.div key="overview" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {statsCards.map((stat, index) => (
                      <motion.div
                        ...
                        transition={{ delay: index * 0.1 }}
                        className={`${getStatBgGradient(stat.color)} rounded-2xl p-6 border border-white shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 relative overflow-hidden`}
                      >
                        <div className="absolute top-0 right-0 w-20 h-20 bg-white/10 rounded-full -mr-10 -mt-10"></div>
                        <div className="relative">
                          <div className="flex items-center justify-between mb-4">
                            <div className={`${getStatIconBg(stat.color)} p-3 rounded-xl shadow-md`}>
                              <stat.icon className="w-6 h-6 text-white" />
                            </div>
                            {stat.realTime && (
                              <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 2, repeat: Infinity }} className="flex items-center space-x-1 bg-white/80 px-2 py-1 rounded-full">
                                <Circle className="w-2 h-2 fill-current text-red-500" />
                                <span className="text-xs font-semibold text-red-600">LIVE</span>
                              </motion.div>
                            )}
                          </div>
                          <p className={`text-sm font-medium ${getStatTextColor(stat.color)} mb-2`}>{stat.label}</p>
                          <p className={`text-4xl font-bold ${getStatValueColor(stat.color)}`}>{stat.value}</p>
                        </div>
                      </motion.div>
                    ))}
                  </div>
                  ...
                  <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
                    <div className="flex items-center justify-between mb-6">
                      <h2 className="text-2xl font-bold text-slate-800 flex items-center">
                        <Calendar className="w-6 h-6 text-blue-600 mr-2" />
                        Today's Sessions
                      </h2>
                      <button onClick={() => handleNavClick('sessions')} className="text-blue-600 hover:text-blue-700 font-medium text-sm flex items-center">
                    ...
                    {todaysSessions.length === 0 ? (
                      <div className="text-center py-12">
                        ...
                        <p className="text-sm text-slate-400 mt-2">Your schedule is clear - enjoy some rest!</p>
                      </div>
                    ) : (
                      <div className="space-y-4">
                        {todaysSessions.slice(0, 3).map((session, index) => (
                          <motion.div
                            key={session._id}
                            initial={{ x: -20, opacity: 0 }}
                            animate={{ x: 0, opacity: 1 }}
                          ...
                          >
                            <div className="flex items-center space-x-4">
                              <div className="w-12 h-12 bg-blue-500 rounded-xl flex items-center justify-center text-white font-bold">
                                {session.patientId?.userId?.name?.charAt(0).toUpperCase() || 'P'}
                              </div>
                              <div>
                                <p className="font-semibold text-slate-800">{session.patientId?.userId?.name || 'Patient'}</p>
                                <p className="text-sm text-slate-600">{new Date(session.scheduledDate).toLocaleTimeString()}</p>
                              </div>
                            </div>
                            <div className="flex items-center space-x-3">
                  ...
                  {/* Quick Actions */}
                  <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.6 }} className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <button onClick={() => handleNavClick('patients')} className="p-6 bg-gradient-to-br from-rose-500 to-pink-600 rounded-2xl shadow-lg hover:shadow-xl transition-all hover:scale-105 text-white">
                      <Heart className="w-8 h-8 mb-3" />
                      <h3 className="font-bold text-lg mb-1">My Patients</h3>
                      <p className="text-sm text-white/80">View {assignedPatients.length} assigned patients</p>
                    </button>
                    <button onClick={() => handleNavClick('sessions')} className="p-6 bg-gradient-to-br from-blue-500 to-cyan-600 rounded-2xl shadow-lg hover:shadow-xl transition-all hover:scale-105 text-white">
                      <Calendar className="w-8 h-8 mb-3" />
                      <h3 className="font-bold text-lg mb-1">Sessions</h3>
                      <p className="text-sm text-white/80">{todaysSessions.length} sessions today</p>
                    </button>
                    <button onClick={() => handleNavClick('feedback')} className="p-6 bg-gradient-to-br from-purple-500 to-violet-600 rounded-2xl shadow-lg hover:shadow-xl transition-all hover:scale-105 text-white">
                      <Star className="w-8 h-8 mb-3" />
                      <h3 className="font-bold text-lg mb-1">Feedback</h3>
                      <p className="text-sm text-white/80">{feedback.analytics?.totalFeedback || 0} reviews</p>
                    </button>
                  </motion.div>
                </motion.div>
              )}
              ...
              ═══════════════════════════════════════════════════════════ */}
             {/* ═══════════════════════════════════════════════════════════
    PATIENTS SECTION - 🔥 PRODUCTION READY WITH MyPatients COMPONENT
═══════════════════════════════════════════════════════════ */}
...
{/* ═══════════════════════════════════════════════════════════
    SESSIONS SECTION - WITH LIVE TRACKING PANEL
═══════════════════════════════════════════════════════════ */}
{activeSection === "sessions" && (
  <motion.div 
    ...
    className="space-y-6"
  >
    {/* Live Tracking Panel - Show if there's an active session */}
    {activeSession && (
      <LiveTrackingPanel 
    ...
    {/* Real-Time Session List with Track Live Handler */}
    <RealtimeSessionList 
      onTrackLive={handleTrackLive}
      onSessionUpdate={handleRefreshSessions}
    />
  </motion.div>
)}
              ...
              ═══════════════════════════════════════════════════════════ */}
            {/* ═══════════════════════════════════════════════════════════
    FEEDBACK SECTION - 🔥 WITH LIVE DATA
═══════════════════════════════════════════════════════════ */}
{activeSection === "feedback" && (
  ...
  >
    ...
    <FeedbackAnalyticsDashboard feedbackData={feedback.feedbacks || []} />
    ...
    {/* Feedback List */}
    <FeedbackList 
      feedbackData={feedback.feedbacks || []} 
      onLoadMore={() => toast.info('Load more coming soon!')}
      hasMore={feedback.pagination?.hasNextPage}
    />
  </motion.div>
)}

              {/* ═══════════════════════════════════════════════════════════
                  PROFILE SECTION - USING TherapistProfileSettings
              ═══════════════════════════════════════════════════════════ */}
